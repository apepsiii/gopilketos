document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        list.innerHTML = candidates.map(candidate => {
            ...
            return `
            <div class="group relative bg-white rounded-2xl p-6 shadow-sm border border-slate-200 hover:shadow-md hover:border-primary/30 transition-all duration-300 cursor-pointer"
                 onclick="window.location.href='/vote/step2?uuid=${encodeURIComponent(uuid)}&chairman_id=${candidate.id}'">
                <div class="flex flex-col items-center text-center gap-4">
                    <div class="relative">
                        <img src="${candidate.photo_url || '/static/images/default-profile.svg'}" alt="${candidate.name}" 
                             class="w-28 h-28 rounded-full object-cover border-4 border-slate-100 group-hover:border-primary/50 transition-colors" />
                        <div class="absolute -bottom-2 -right-2 w-8 h-8 bg-primary rounded-full flex items-center justify-center shadow-lg">
                    ...
                    <div class="w-full pt-4 border-t border-slate-100 space-y-3">
                        <div class="text-left">
                            <p class="text-xs font-bold uppercase tracking-wider text-primary mb-1">Visi</p>
                            <p class="text-sm text-slate-600 line-clamp-2">${candidate.vision}</p>
                        </div>
                        <div class="text-left">
                            <p class="text-xs font-bold uppercase tracking-wider text-primary mb-1">Misi</p>
                            <p class="text-sm text-slate-600 line-clamp-2">${candidate.mission}</p>
                        </div>
                    </div>
                    <div class="w-full mt-2">
                        <span class="inline-flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-lg text-sm font-semibold group-hover:bg-primary/90 transition-colors">
        ...
        if (window.Html5QrcodeScanner && candidateScanner) {
            ...
            scanner.render((decodedText) => {
                ...
                if (candidateMap[id]) {
                    ...
                    window.location.href = `/vote/step2?uuid=${encodeURIComponent(uuid)}&chairman_id=${encodeURIComponent(id)}`;
                } else if (scannerStatus) {
                    scannerStatus.textContent = 'QR kandidat tidak valid. Pastikan Anda memindai QR kode ketua.';
                }
            }, () => {
                // ignore scan errors
            });
